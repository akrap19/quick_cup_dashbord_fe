...
import { createRoomFull } from 'api/services/content/rooms'
// import { useManageContentAdd } from '@/store/manage-content-add'
...
export const PreviewAndPublish = ({ languages }: Props) => {
	...
	const { rooms, imagesToDisplay, audioToDisplay, setRooms } = useManageContentAdd()
	...
	const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
		...
		if (rooms) {
			const transformedData = {
				translations: rooms?.map(room => ({
					languageId: replaceEmptyStringWithNull(room.languageId),
					title: replaceEmptyStringWithNull(room.title),
					description: replaceEmptyStringWithNull(room.description),
					audioId: replaceEmptyStringWithNull(room.audioId)
				})),
				images: rooms[rooms?.length - 1]?.images || [],
				deletedImages: rooms[0]?.deletedImages || []
			}

			const result = await createRoomFull(transformedData)

			if (result?.message === 'OK') {
				setRooms({} as ContentPayload)
					...
					<Box paddingX={6} paddingTop={6} borderTop="thin" borderColor="neutral.300">
						{rooms && rooms[0] ? (
							<>
								...
								<MobilePreview
									content={{ rooms: roomToShow?.filter(room => room.languageId === currentLanguage.id) as any[] }}
