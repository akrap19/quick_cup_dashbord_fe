...
import { deleteAdditionalCost, deleteAdditionalCosts } from 'api/services/additionalCosts'
...
export const Inputs = ({ data }: Props) => {
	...
	const handleEdit = () => {
		...
		push(ROUTES.EDIT_ADDITIONAL_COSTS + data[numericIndex]?.id)
		refresh()
	}
	...
	const handleDelete = async () => {
		...
		const result = await (isDeleteBulk ? deleteAdditionalCosts(ids) : deleteAdditionalCost(ids[0]))

		if (result?.message === 'OK') {
			SuccessToast(t(isDeleteBulk ? 'AdditionalCosts.successfulBulkDelete' : 'AdditionalCosts.successfullyDeleted'))
			clearCheckedItems()
				...
				<Inline justifyContent="space-between" alignItems="center">
					<Box style={{ width: '320px' }}>
						<SearchInput
							...
							placeholder={t('AdditionalCosts.search')}
							onChange={({ target: { name, value } }) => debouncedFilterChange(name, value)}
						/>
					</Box>
					<AddButton buttonLabel={t('AdditionalCosts.add')} buttonLink={ROUTES.ADD_ADDITIONAL_COSTS} />
				</Inline>
			) : (
				<DataTableActions onEdit={handleEdit} onDelete={() => confirmDialog.toggleOpened()} />
			)}
			<ConfirmActionDialog
				title="AdditionalCosts.delete"
				description="AdditionalCosts.deleteDescription"
