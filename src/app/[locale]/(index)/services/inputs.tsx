...
import { deleteService, deleteServices } from 'api/services/services'
...
export const Inputs = ({ data }: Props) => {
	...
	const handleEdit = () => {
		...
		push(ROUTES.EDIT_SERVICES + data[numericIndex].userId)
		refresh()
	}
	...
	const handleDelete = async () => {
		...
		const result = await (isDeleteBulk ? deleteServices(ids) : deleteService(ids[0]))

		if (result?.message === 'OK') {
			SuccessToast(t(isDeleteBulk ? 'Services.successfullBulkDelete' : 'Services.successfullyDeleted'))
			clearCheckedItems()
		...
		<div>
			{checkedItemsLength === 0 ? (
				<Inline justifyContent="space-between" alignItems="center">
					...
					<AddButton buttonLabel={t('Services.add')} buttonLink={ROUTES.ADD_SERVICES} />
				</Inline>
			) : (
			...
			<ConfirmActionDialog
				...
				description="Services.deleteServiceDescription"
